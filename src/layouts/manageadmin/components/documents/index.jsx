...
const scanColor = (s) => (s === 'Clean' ? 'success' : (s === 'Infected' || s === 'Failed') ? 'error' : s === 'Quarantined' ? 'warning' : 'secondary');
const cap = (v) => <ArgonTypography variant="caption" color="secondary">{v ?? '-'}</ArgonTypography>;
...
function ManageDocuments() {
  ...
  const [ctx, setCtx] = useState({ library: false, expiring: false, types: false });
  const [docs, setDocs] = useState([]);
  const [expiring, setExpiring] = useState([]);
  const [types, setTypes] = useState([]);
  const [typeOpen, setTypeOpen] = useState(false);
  const [confirm, setConfirm] = useState({ open: false, id: null });
  const [filters, handleFilterChange] = useForm({});
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        message={t('generic.confirmDelete', { defaultValue: 'Are you sure?' })}
