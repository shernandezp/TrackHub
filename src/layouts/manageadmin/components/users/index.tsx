...
function ManageUsers() {
  ...
  const handleEditClick = (rowData: UserFormValues) => {
    setUserValues(rowData);
    setUserErrors({});
  };

  const handleUpdatePasswordClick = (rowData: PasswordFormValues) => {
  ...
  const {
    ...
    setOpen,
    setOpenPassword,
    setConfirmOpen} = useUserTableData(
      expanded,
      handleEditClick,
  ...
  return (
    <>
      <TableAccordion
        ...
        <Table columns={columns} rows={rows} selectedField='user' serverPaged />
        <ServerPagination
          page={page}
          pageSize={PAGE_SIZE}
