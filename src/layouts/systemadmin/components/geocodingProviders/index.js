...
function ManageGeocodingProviders() {
  ...
  const handleDeleteClick = (geocodingProviderId) => {
  ...
  const handleSubmit = async () => {
    let requiredFields = ['name', 'type', 'endpointUri'];

    if (validate(requiredFields)) {
      onSave({
        ...values,
        requestsPerSecond: parseInt(values.requestsPerSecond, 10) || 1,
        timeoutSeconds: parseInt(values.timeoutSeconds, 10) || 5
      });
    }
  };
