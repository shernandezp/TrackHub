 ...
 * @param {number} colorValue - Color value stored on the POI.
 * @returns {string} Hex color.
 */
export const getPoiColorHex = (colorValue) =>
  COLOR_LABEL_HEX[getColor(colorValue)] || DEFAULT_POI_COLOR;

/**
 * Returns the i18n key suffix for a POI type value (e.g. 1 -> 'clientSite').
 * @param {number} typeValue - PointOfInterestType value.
 * @returns {string} Camel-cased type key or empty string.
 */
export const getPoiTypeKey = (typeValue) => {
 ...
 * @param {string} colorHex - Fill color.
 * @param {string} format - 'svg' or 'dataURL'.
 * @returns {string} SVG markup or data URL.
 */
export const createPoiPinSvg = (colorHex, format = 'svg') => {
 ...
 * @param {Object} poi - PointOfInterestVm.
 * @param {Function} t - Translation function.
 * @returns {string} HTML string.
 */
export const buildPoiPopupHtml = (poi, t) => {
  const typeKey = getPoiTypeKey(poi.type);
  const typeLabel = typeKey ? t(`poi.types.${typeKey}`) : '';
  let content = `
