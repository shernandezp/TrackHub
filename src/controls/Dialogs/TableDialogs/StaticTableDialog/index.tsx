...
interface StaticTableDialogProps {
    title: ReactNode;
    children?: ReactNode;
    handleSave: (selectedRows: number[], checked: boolean) => void | Promise<void>;
    open: boolean;
            ...
            <DialogActions>
                <Box display="flex" justifyContent="space-between" width="90%">
                    {checkLabel && (
                        <FormControlLabel
                            control={
                                <Checkbox checked={checked} onChange={(event: ChangeEvent<HTMLInputElement>) => setChecked(event.target.checked)} />
                            }
                            label={checkLabel}
                        />
                    )}
