...
export function extractRouteKeys(routesSource) {
  const keys = [];
  const entryPattern =
    /type:\s*["'](route|hidden|title|divider|collapse)["'][\s\S]*?key:\s*["']([^"']+)["']/g;
  let match;
...
export function validateHelpContent({ helpDir, routesFile }) {
  ...
  for (const lang of languages) {
    ...
    for (const name of files) {
          ...
          for (const screen of data.screens) {
            if (!routeKeys.includes(screen)) {
              addError(
                file,
                1,
                `screen '${screen}' is not a route key in routes.tsx (known: ${routeKeys.join(", ")})`
              );
            }
          }
        }
      ...
      if (h1s.length !== 1) {
        addError(
          file,
          h1s.length ? lineOf(body, h1s[1].index) : null,
          `topic must have exactly one H1 (found ${h1s.length})`
        );
      } else if (typeof data.title === "string" && h1s[0][1].trim() !== data.title.trim()) {
        addError(
          file,
          lineOf(body, h1s[0].index),
          `H1 '${h1s[0][1].trim()}' must equal frontmatter title '${data.title}'`
        );
      }
      ...
      while ((htmlMatch = htmlPattern.exec(codeless)) !== null) {
        addError(
          file,
          lineOf(codeless, htmlMatch.index),
          `raw HTML is not allowed: '${htmlMatch[0]}'`
        );
      }
      ...
      while ((linkMatch = linkPattern.exec(body)) !== null) {
        ...
        if (/\.md(#[^)]*)?$/i.test(target) && !/^https?:/i.test(target)) {
          addError(
            file,
            line,
            `relative .md link '${target}' is not allowed — use the topic: scheme`
          );
        }
      }
      ...
      for (const field of ["category", "order", "featureKey"]) {
        if (JSON.stringify(other.data[field] ?? null) !== JSON.stringify(en.data[field] ?? null)) {
          addError(
            other.file,
            1,
            `'${field}' differs from the en file (structural fields must be identical across languages)`
          );
        }
      }
    ...
    for (const lang of Object.keys(topic.perLang)) {
      ...
      while ((match = topicLinkPattern.exec(body)) !== null) {
        if (!topics.has(match[1])) {
          addError(
            file,
            lineOf(body, match.index),
            `topic: link target '${match[1]}' does not exist`
          );
        }
      }
  ...
  const coveredScreens = new Set(
    [...topics.values()].flatMap((topic) =>
      Object.values(topic.perLang).flatMap(({ data }) =>
  ...
  for (const key of routeKeys) {
    if (!coveredScreens.has(key)) {
      addError(
        routesFile,
        null,
        `route key '${key}' has no help topic (every screen needs ≥1 topic whose screens include it)`
      );
    }
  }
